...
def run_pg(render=False):
    ...
    with tf.Session() as sess:
        ...
        for episode in range(max_episodes):
            ...
            if solved:
                return solved, episode, average_rewards, p_loss

            # Compute Rt for each time-step t and update the network's weights
            for t, transition in enumerate(episode_transitions):
                total_discounted_return = sum(discount_factor ** i * tr.reward for i, tr in enumerate(episode_transitions[t:])) # Rt
...
def multiple(n_iters):
    ...
    for _ in range(n_iters):
        start = time.time()
        solved, curr_ep, _, p_loss = run_pg()
        end = time.time()
        if solved:
