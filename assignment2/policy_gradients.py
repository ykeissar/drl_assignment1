...
class PolicyNetwork:
    def __init__(self, state_size, action_size, learning_rate, name='policy_network'):
        ...
        with tf.compat.v1.variable_scope(name):
            ...
            self.W1 = tf.compat.v1.get_variable("W1", [self.state_size, 12], initializer=tf.contrib.layers.xavier_initializer(seed=0))
            self.b1 = tf.compat.v1.get_variable("b1", [12], initializer=tf.zeros_initializer())
            self.W2 = tf.compat.v1.get_variable("W2", [12, self.action_size], initializer=tf.contrib.layers.xavier_initializer(seed=0))
