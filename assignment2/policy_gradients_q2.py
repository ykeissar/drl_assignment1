...
def run_pg(discount_factor=0.999, learning_rate=0.0002, v_learning_rate=0.005, render=False):
        ...
        for episode in range(max_episodes):
            ...
            for step in range(max_steps):
                ...
                if done:
                    ...
                    v_s_tag = value.predict(next_state, sess)

                v_s = value.predict(state, sess)
                delta = reward + discount_factor * v_s_tag - v_s

                action_one_hot = np.zeros(action_size)
                action_one_hot[action] = 1

                _, loss = value.update(state, reward + discount_factor*v_s_tag, sess)
                v_loss.append(loss)
                _, loss = policy.update(state, I * delta, action_one_hot, sess)
                p_loss.append(loss)
                if done:
                    break
...
def only_one():
    ...
    if solved:
        with open('losses/ac_p.json', 'w+') as f:
            json.dump([str(p) for p in p_loss], f)

        with open('losses/ac_v.json', 'w+') as f:
            json.dump([str(v) for v in v_loss], f)
